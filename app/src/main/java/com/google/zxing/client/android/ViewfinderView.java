...
/**
 *此视图覆盖在相机预览的顶部。 它增加了取景器矩形和部分透明度，以及激光扫描仪的动画和结果点。
...
public final class ViewfinderView extends View {
  ...
  public ViewfinderView(Context context, AttributeSet attrs) {
    ...
    resultColor = resources.getColor(R.color.jieguo);
  ...
  public void onDraw(Canvas canvas) {
    ...
    Rect frame = cameraManager.getFramingRect();//获取矩形框（根据屏幕分辨率然后按照一个固定的比例来设置方框大小）
    Rect previewFrame = cameraManager.getFramingRectInPreview();// 获取扫描区域的矩形框（getFramingRect是UI的显示，这是扫描的区域，其实竖屏扫描的时候，形式是正方形的框，但是获取到的图片确实长方形的）
